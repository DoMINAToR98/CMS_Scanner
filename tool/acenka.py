...
def scan(cmslist):
	...
	if(target[0][1]=="WordPress"):
		...
		subprocess.call("wpscan --url "+parameter+" --no-banner --detection-mode aggressive -o target_report.txt".split()) 
	elif(target[0][1]=="Joomla"):
		print "!---Starting Joomla Scan---!"
		subprocess.call(("joomscan -u "+parameter+" > target_report.txt").split())
	elif(target[0][1]=="Drupal"):
		print "!---Starting Drupal Scan---!"
		subprocess.call(("droopescan scan drupal -u "+parameter+" > target_report.txt").split())
	else: 
